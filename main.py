...
# 尝试从 requests.packages.urllib3 导入并忽略 InsecureRequestWarning
# 这通常在 requests 库中使用 verify=False 时出现
try:
    from requests.packages.urllib3.exceptions import InsecureRequestWarning
    warnings.simplefilter('ignore', InsecureRequestWarning)
except ImportError:
    # 如果无法导入，可能意味着 requests 版本不同或未安装，忽略此警告设置
    pass

# --- 模块导入 ---
# 导入本地模块中的函数
# 假设模块 (processor, utils) 与 main.py 在同一个包或目录下。
# 如果作为脚本直接运行，Python 可能需要帮助找到这些模块。
try:
    # 优先尝试相对导入（适用于作为包运行时）
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog='示例: python main.py -u https://example.com -o results.txt' # 添加使用示例
    )
    ...
    try:
        args = parser.parse_args()
         ...
         # 在这里可以捕获它，或者让程序直接退出
         # print("参数错误，请检查输入。", file=sys.stderr)
    ...
    try:
        if args.url:
            # 处理单个网页 URL
            # 创建一个空的 JS URL 缓存，供 process_web_page 及其调用的 process_js_url 使用
            js_cache = set()
            processor.process_web_page(args.url, output_file_path, js_cache)
        elif args.extract_url:
            # 处理单个 JS URL
            js_cache = set() # 虽然只处理一个 URL，但保持接口一致性
        ...
        # 此处不需要 else，因为 input_group 设置了 required=True，argparse 会确保至少有一个被选中
        ...
        # 无论成功还是失败，都打印结束信息
        print(f"\n分析完成。结果已（部分）保存到: {output_file_path}")
